...
class DigestClient {
    ...
    addAuth(url, options) {
        ...
        const ha1 = md5(`${this.user}:${this.digest.realm}:${this.password}`).toString()
        const ha2 = md5(`${method}:${uri}`).toString()
        const ncString = ('00000000' + this.digest.nc).slice(-8)
        const response = md5(`${ha1}:${this.digest.nonce}:${ncString}:${this.digest.cnonce}:${this.digest.qop}:${ha2}`).toString()
